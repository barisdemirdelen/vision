function bonus_predict()
% This function predicts probabilities for all SIFT Methods and averages
% them to obtain the bagging result.
cl_rgb = 'rgb';
cl_hsv = 'hsv';
cl_opp = 'opponent';
cl = cl_opp;
...
for k=1:4
    ...
    for j=1:3
        ...
        for i=1:1
            ...
            prediction = predict_for_classifier(centers, cl, k, all_images_valid, samplesize);
            predictions = horzcat(predictions, prediction);
        end
    end
    disp(size(predictions))
    mean_predictions = mean(predictions,2);
    disp(size(mean_predictions))
    labels = [];
    for m=1:4
        for n=1:samplesize
            if m == k
    ...
    [map, ids]= get_map(mean_predictions, labels, samplesize)
    aps = [aps; map];
    image_list = [];
...
disp(image_matrix);
    ...
    write_html(total_map, aps, image_matrix);
end

function write_html(total_map, aps, image_matrix)
fileID = fopen(strcat('results/Result-bonus.html'),'w');
...
fprintf(fileID,strcat('<tr><th>SIFT method</th><td> rgb, hsv and opponent</td></tr>\n'));
fprintf(fileID,strcat('<tr><th>Vocabulary size</th><td>400 words', '</td></tr>\n'));
...
function probs = predict_for_classifier(centers, cl, classifier, test_images, samplesize)
